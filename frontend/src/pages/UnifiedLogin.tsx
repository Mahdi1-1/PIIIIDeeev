...
export function UnifiedLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend returns active_theme_id upon login, but frontend uses localStorage to determine initial theme
      // You should probably set the theme in AuthContext or here if needed.
      
      const isAdmin = email.includes('admin') || email.includes('mod') || email.includes('mentor') || email.includes('enterprise');
      if (isAdmin) {
        navigate('/admin');
      } else {
        // Redirection vers le backoffice utilisateur (dashboard)
        navigate('/dashboard');
      }
  ...
  const handleQuickLogin = async (role: UserRole) => {
    ...
    setTimeout(async () => {
      ...
      //   navigate('/admin');
      // }
      setError("Mock login disabled. Please use real credentials.");
      setLoading(false);
    }, 500);
  };
